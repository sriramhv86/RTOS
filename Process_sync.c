...
int main(int argc, char *argv[])
{
	...
	key_t key;
	pid_t pid1, pid2;
	mutex = sem_open("/semfork", O_CREAT|O_EXCL, 0644, 1);
	if(mutex == SEM_FAILED){
	...
	if(getpid()!=parentid){
		...
		mutex = sem_open("/semfork", 1);
		if(mutex == SEM_FAILED){
			perror("client: unable to execute semaphore");
			sem_close(mutex);
			exit(-1);
		}
		//childpid = getpid();
		...
		for(j=0; j<=199; j++){
			...
			rc = gettimeofday(&tod1, NULL);// This the changed part
			sem_wait(mutex);
			temp[j] = shm[j];
